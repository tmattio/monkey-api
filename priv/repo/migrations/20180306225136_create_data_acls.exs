defmodule Monkey.Repo.Migrations.CreateDataAcls do
  ...
  def change do
    create table(:data_acls) do
      add(:type_id, references(:data_types, on_delete: :nothing))
    ...
    create(index(:data_acls, [:type_id]))
    create(index(:data_acls, [:image_id]))
    create(index(:data_acls, [:video_id]))
    create(index(:data_acls, [:text_id]))
    create(index(:data_acls, [:dataset_id]))
  end
end
